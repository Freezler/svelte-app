<script>
	...
	const reloadAvatar = async () => {
		isLoading = true;
		const response = await fetch('https://source.unsplash.com/360x360/?svelte, tailwindcss');
...
<main class="flex h-screen w-screen animate-slow-fade-in flex-col items-center justify-center bg-gradient-to-tr from-astral-900 via-gray-900 to-astral-900">
	...
	<button on:click={reloadAvatar} class=" mt-8 rounded-md border-2 border-purpleone-700 bg-purpleone-900 bg-opacity-40 p-3 text-rosetwo-200 opacity-80 shadow-sm shadow-purpleone-500">Refresh Avatar</button>
</main>