<script>
	...
	const toggleNav = () => {
	 ...
	 // Reset hamburger transformations
    const hamburgerIcon = document.getElementById('hamburgerIcon');
    if (hamburgerIcon) {
      hamburgerIcon.classList.remove('transform', 'rotate-45', '-rotate-45', 'translate-y-1', '-translate-y-1');
    }
  };
</script>
...
<button id="hamburgerIcon" class="lg:hidden mr-7 animate-burgersmooth" on:click={toggleNav}>
  <div
    class={`w-7 h-1 bg-[#EF7FFF] my-1 ${
      showNav ? 'transform rotate-45 transition duration-300 ease-in-out translate-y-1' : ''
    }`}
  />
  <div class={`w-7 h-1 bg-[#EF7FFF] my-1 ${showNav ? 'hidden' : ''}`} />
  <div
    class={`w-7 h-1 bg-[#EF7FFF] my-1 ${
...
<!-- ... rest of the code ... -->
Save to grepper
In this updated code, we're targeting the hamburger icon by its ID (hamburgerIcon) and removing the transformation classes (transform, rotate-45, -rotate-45, translate-y-1, -translate-y-1) when closing the menu. This will animate the hamburger icon back to its original state.
...
>
	...
	<!-- Hamburger Icon for Small Screens -->
	<button class="lg:hidden mr-7 animate-burgersmooth" on:click={toggleNav}>
		<div
			class={`w-7 h-1 bg-[#EF7FFF] my-1 ${
				showNav
		...
		<div
			class={`w-7 h-1 bg-[#EF7FFF] my-1 ${
				showNav
					? 'transform -rotate-45 transition duration-300  -translate-y-1'
					: ''
			}`}
		/>
	</button>
