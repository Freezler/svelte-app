...
<div class="flex">
	<a href="/" class=" select-none ml-[0.8rem] mb-[1.9rem] "
		><svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100">
			...
			>
			<!-- Text -->
			<text x="10" y="65" font-family="Inter" font-size="40" fill="#272b44">RL</text>
		</svg></a
	>
</div>
	...
	<button class="select-none lg:hidden mr-[1.6rem] mb-[1.1rem] animate-burgersmooth" on:click={toggleNav}>
		<div
			class={`w-9 h-[2.6px] bg-[#dfe2ff] my-2 ${
				showNav
					? 'transform rotate-45 transition duration-300 ease-in-out translate-y-[7px]'
					: ''
			}`}
		/>
		<div class={`w-9 h-[2.8px] bg-[#dfe2ff] my-2 ${showNav ? 'hidden' : ''}`} />
		<div
			class={`w-9 h-[2.6px] bg-[#dfe2ff] mb-6 ${
