<script>
	...
	const reloadAvatar = async () => {
		isLoading = true;
		const response = await fetch('https://source.unspladsh.com/2169x2169/?beautdiful');
...
>
	<div class="justify-top flex w-100vh flex-col items-center border-b-[1px] bg-black">
		...
		{:else if avatarUrl !== ''}
			<div
				class="  drop-shadow-[2px_2px_12px_hsla(54,50%,99%,0.5)]] avatar-image flex h-screen w-[100%] min-w-[400px] max-w-[1440px] scale-100 flex-col items-center justify-center  bg-cover bg-center text-center md:scale-[1]"
			>
				<Hero />
				<Header />
			</div>
