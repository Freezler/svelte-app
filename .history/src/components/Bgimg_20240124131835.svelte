<script>
	...
	const reloadAvatar = async () => {
		isLoading = true;
		const response = await fetch('https://source.unsplash.com/2169x2169/?bea');
...
>
	<div class="justify-top flex w-full flex-col items-center border-b-[1px] bg-black">
		...
		{:else if avatarUrl !== ''}
			<div
				class="  drop-shadow-[2px_2px_12px_hsla(134,50%,9%,0.9)]] avatar-image flex h-screen w-[100%] min-w-[100%] max-w-[100%] scale-100 flex-col items-center justify-center  bg-cover bg-center text-center md:scale-[1]"
			>
				<Hero />
				<Header />
			</div>
...
.avatar-image:has(img) {
	animation: bg-zoom 3000 ease-in-out infinite;
}
</style>