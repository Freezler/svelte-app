<script>
	...
	const reloadAvatar = async () => {
		isLoading = true;
		const response = await fetch('https://source.unsplash.com/100x100/?dark,pattern');
		avatarUrl = ``;
		isLoading = false;
	};
	...
	function handleClick() {
		document.querySelector('.avatar-image').style.backgroundImage = `color: #000`;
	}
	const setAvatar = () => {
		let ava = document.querySelector('.avatar-image');
		ava.style.backgroundImage = `url(${avatarUrl})`;
	}
	setAvatar.onClick = handleClick;
</script>
