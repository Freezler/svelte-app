...
{#if show}
	<div
		id="container"
		class="relative mx-auto flex h-[50vh] w-[60%] items-center justify-center overflow-hidden rounded p-[3rem] transition duration-[600ms]
		"
	>
		{#each info as item}
			<!-- Slide Container -->
			<div
				class="slide cubic-bezier(0.075, 0.82, 0.165, 1) flex h-[100] w-[100vw] flex-col items-center justify-center overflow-hidden transition-all duration-[1600ms] hover:w-[2200px]"
			>
				<!-- image Container -->

				<div
					class=" image flex transform flex-col items-center justify-around overflow-hidden ease-in-out"
				>
					<img
						src={item.image}
						class=" object-fit h-[100vh] w-[100vw] transform object-top transition duration-[1400ms]"
						alt=""
						loading="lazy"
					/>
					<!-- Overlay Container -->
