...
>
	...
	<button class="lg:hidden mr-7 animate-burgersmooth" on:click={toggleNav}>
		...
		<div
			class={`w-7 h-1 bg-[#EF7FFF] my-1 ${
				showNav ? 'transform -rotate-45 transition duration-300  -translate-y-1' : ''transform -rotate-45 transition duration-300  -translate-y-1' '
				not shownav ? 'transform -rotate-45 transition duration-300  -translate-y-1' : ''
			}`}
		/>
	</button>
