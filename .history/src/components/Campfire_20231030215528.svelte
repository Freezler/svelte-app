...
<main class=" flex h-screen w-screen animate-slow-fade-in flex-col items-center justify-center pt-[10%] gap-8 bg-gradient-to-tr from-astral-900 via-gray-900 to-astral-900">
	{#if isLoading}
	<div class=" border-purplethree-400 h-[60%] max-h-[70%] w-[60%] max-w-[60%] animate-pulse border-2 rounded-[8px] flex items-center justify-center">
		<p class="text-xl text-purpleone-100 animate-pulse">Loading...</p>
	</div>
		{:else if avatarUrl !== ''}
		
		<div class="flex mb-8  min-w-[80%] w-[60%] max-w-[60%] min-h-[40%] h-[60%] max-h-[70%] flex-col rounded-[8px] border-2 border-purplethree-400 bg-cover bg-center avatar-image" style="background-image: url({avatarUrl})"></div>
	{/if}
