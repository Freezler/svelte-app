<script>
  ...
  <div id="flex-1 w-[100vw] h-[100%] items-center absolute top-0 left-0 right-0	overflow-hidden	 ">
  {#each info as item}
  <div class="slide transition-all relative flex-1 overflow-hidden">
	<div class="image transition-transform ease-in-out duration-500 " style="background-image: url({item.image});"></div>
	<div class="overlay absolute top-0 left-0 right-0 bottom-0">
	  <div class="content">
		<h1 class="title" data-title={item.city}>{item.city}</h1>
		<div class="emblem hover:transform hover:-translateX-100" style="background-image: url({item.emblem});"></div>
		<ul class="city-info">
