...
<main class="overflow-hidden flex items-center justify-center mx-auto  h-[100vh] w-screen animate-slow-fade-in flex-col bg-gradient-to-tr from-astral-900 via-gray-900 to-astral-900">
  <div class="border-b-[1px] border-purpleone-600 flex flex-col items-center justify-top  w-full">
	...
	{:else if avatarUrl !== ''}
	<div  ty class="h-[100vh] z-1000   bg-shadow-[2px_2px_12px_[rgba(22,22,22,0.7] text-center drop-shadow-[2px_2px_12px_hsla(234,100%,59%,0.9)]] avatar-image flex items-center justify-top w-[100%] min-w-[100%] max-w-[100%] flex-col border-[1px] border-bluetwo-600 bg-cover bg-center drop-shadow-[2px_2px_12px_hsla(255,100%,59%,0.9)]">
		<button type="button" on:click={handleClick} class="text-purpleone-300  bg-gray-950 	py-2 px-3 mt-8 hover:text-purpleone-100 z-1000">check</button>	  <Hero />
	</div>
	{/if}
  </div>
</main>