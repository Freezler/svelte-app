...
<script>
  ...
  const toggleNav = () => {
    ...
    if (hamburgerIcon) {
      hamburgerIcon.classList.remove('transform', 'rotate-45', '-rotate-45', 'translate-y-0', '-translate-y-0');
    }
  };
</script>
...
<button id="hamburgerIcon" class="lg:hidden mr-7 " on:click={toggleNav}>
	<div
	  class={`w-7 h-1 bg-[#EF7FFF]  ${
		showNav ? 'transform rotate-45 transition duration-300 ease-in-out translate-y-1' : ''
	  }`}
	/>
	<div class={`w-7 h-1 bg-[#EF7FFF] ${showNav ? 'hidden' : ''}`} />
	<div
	  class={`w-7 h-1 bg-[#EF7FFF]  ${
