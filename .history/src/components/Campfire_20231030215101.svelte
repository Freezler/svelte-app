...
<main class="translate-y-[150%] flex h-screen w-screen animate-slow-fade-in flex-col items-center justify-top pt-[10%] gap-8 bg-gradient-to-tr from-astral-900 via-gray-900 to-astral-900">
	{#if isLoading}
	<div class=" border-purplethree-400 h-[45%] w-[60%] animate-pulse border-2 rounded-[8px] flex items-center justify-center">
		...
		<div class="flex h-[45%] min-w-[60%] w-[60%] min-h-[40%] max-h-[70%] flex-col rounded-[8px] border-2 border-purplethree-400 bg-cover bg-center avatar-image" style="background-image: url({avatarUrl})"></div>
	{/if}
