...
<div id="container" class="absoulute top-0 left-0 right-0 w-screen h-screen flex items-center">
  ...
  <div class="slide relative top-0 left-0 right-0 w-flex-1 h-[200px] overflow-hidden">
	...
	<div class="overlay absolute top-0 left-0 right-0 w-full h-full flex items-center justify-center bg-transparent opacity-0 transition-opacity duration-500 ease-in-out hover:opacity-100">
	  <div class="opacity-100 transform-absolute top-0 left-0 right-0 w-full h-full flex flex-col items-center justify-center duration-500 ease-in-out -translate-x-full hover:translate-x-0">
		<h2>{item.city}</h2>
		<h3>{item.country}</h3>
		<p>{item.population}</p>
		<p>{item.founded}</p>
