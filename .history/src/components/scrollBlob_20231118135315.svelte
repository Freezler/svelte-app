...
>
	{#each info as item}
		<!-- Slide Container -->
		<div
			class="slide relative grid h-[100%] w-[container]  grid-cols-1 grid-rows-1 place-content-center justify-items-center overflow-hidden active:scale-125 transition-all  duration-[1900ms] ease-in-out cubic-bezier(0.075, 0.82, 0.165, 1) hover:w-[1500%] "
		>
			<!-- image Container -->
			...
			<div  class=" image h-[100vh] w-[100vw]  transform overflow-hidden">
				<img
					src={item.image}
					class="relative h-[100%] w-[100vw] object-fill hover:scale-125 transition-all duration-[3000ms] cubic-bezier(0.075, 0.82, 0.765, 1)"
					alt=""
					loading="lazy"
				/>
				<!-- Overlay Container -->
				<div
					class="overlay absolute inset-0  translate-x-[100%]  translate-y-full  flex w-[100vw] h-[100vh] items-center justify-center"
				>
					<!-- Content Container -->
					<div
						class="relative content flex flex-1 opacity-0  gap-8 flex-col items-center justify-between text-left font-display text-white  shadow-2xl transition-all duration-[1900ms] ease-in-out text-shadow-[3px_3px_5px_hsla(0,0%,0%,0.9)]"
					>
						<h1
						...
						<ul
							class="city-info opacity-1 z-100 flex transform  flex-1 flex-row   font-display transition-all duration-[3000ms] ease-in-out"
						>
...
<style>
	* {
	...
	.emblem {
		transform:  scale(0.5) opacity(0) rotate(0) skew(0) rotate3d, 1, 12, 120deg;
