<script>
  ...
  const reloadAvatar = async () => {
	isLoading = true;
	const response = await fetch('https://source.unsplash.com/1440x1200/?abstract?art');
...
<main class="overflow-hidden flex items-center justify-center mx-auto  h-[100vh] w-screen animate-slow-fade-in flex-col bg-gradient-to-tr from-astral-900 via-gray-900 to-astral-900 ">
  <div class="border-b-[1px] border-purpleone-600 flex flex-col items-center justify-top  w-full">
	{#if isLoading}
	<div class="shadow-[3px_3px_15px_3px_rgba(22,22,22,0.7)] flex h-screen bg-cover bg-center drop-shadow-[2px_2px_12px_hsla(255,100%,59%,0.9)] w-[100%] min-w-[100%] max-w-[100vw] animate-pulse items-center justify-center  ">
	  <p class="pt-8 animate-pulse text-md text-purpleone-100"><Loader />Loading</p>
	</div>
	{:else if avatarUrl !== ''}
	<div class="  z-1000 scale-100 md:scale-[1.1] h-screen text-center drop-shadow-[2px_2px_12px_hsla(234,100%,59%,0.9)]] avatar-image flex items-center justify-center w-[100%] min-w-[100%] max-w-[100%] flex-col bg-cover bg-center bg-[2px_2px_12px_hsla(255,100%,89%,1)]">
			  <Hero />
	</div>
	{/if}
  </div>
