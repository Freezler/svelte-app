...
<div class="p-4 min-w-[340px] text-center">
	<input
	  type="file"
	  accept=".txt, .doc, .docx"
	...
	/>
	<textarea bind:value={fileContent} class="text-center text-white h-[2.4rem] focus:outline-none resize-none w-[100%] min-w-[content] bg-transparent border-[1px] select-none border-gray-300 rounded-b-[2px] bor p-2"></textarea>
  
	{#if isEmpty}
	  <h3 class="w-[300px] text-red-500 mt-2 mx-auto px-3 py-1">Selecteer een bestand</h3>
	{:else if isSuccess && gemiddeldeCijfer !== ''}
