<script>
	...
	afterUpdate(() => {
		...
		image.style.transition = 'opacity 0.3s ease-in-out, filter 0.3s ease-in-out, transform 0.3s ease-in-out';
...
>
	<div class="justify-top flex w-full flex-col items-center border-b-[1px] bg-black">
		{#if isLoading}
			...
			>
				<p class="text-sm animate-pulse pt-8 font-display text-purplethree-100"><Loader />Loading</p>
			</div>
		{:else if avatarUrl !== ''}
			<div
